...
// Body JSON : { imageBase64: string, category: string }
...
export async function POST(req: NextRequest) {
  ...
  const categoryLabels: Record<string, string> = {
    ...
    telephones: 'Téléphonie (smartphones, accessoires)',
    ordinateurs: 'Informatique (laptops, ultrabooks)',
    tablettes: 'Tablettes (iPad, tablettes Android)',
  };
  const categoryLabel = categoryLabels[category] || category;

  const prompt = `Tu es un expert en e-commerce spécialisé dans la rédaction de fiches produit.
...
Réponds UNIQUEMENT avec un objet JSON valide (sans markdown, sans backticks, sans commentaires) :
{
  ...
  "description": "Description commerciale complète en français (150-250 mots). Décris les caractéristiques visuelles, la matière supposée, le style, les occasions de port/usage. Utilise un ton éditorial moderne.",
